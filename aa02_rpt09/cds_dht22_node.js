...
<hr>
  ...
  /* 다자스크립트부분 */
    ...
    var tArray = [], // time of data arrival /50개의 시간이 저장됨/
    ...
    socket.on('connect', function () {
        socket.on('message', function (msg) {
            ...
            if (dtda[1] != preX || dtda[2] != preY) {  // any change? //온도 값 조도값이 이전 값과 다르면//
                preX = dtda[1];  
                preY = dtda[2];
    ...
    function init() {  // initial screen ()
        ...
        for ( i = 0; i < numPts; i++) {
            ...
            yTrack.push(data[2]);  // sensor 2 (lux)
        }

        Plotly.plot(streamPlot, data, layout); //변화가있으면 그래프로 갱신 //
    }
